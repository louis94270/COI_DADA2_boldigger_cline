...
FWD <- c("GGDACWGGWTGAACWGTWTAYCCHCC")

REV <- c("CAAACAAATARDGGTATTCGDTY")
...
to.rm <- as.numeric(lapply(merged_list,
                            function(x) sum(x$abundance)))
to.kp <- !(to.rm == 0)
...
asv_headers <- vector(dim(seqtab_nochim)[2],
                      mode = "character")
...
tax <- tax %>%
  replace_na(list(Phylum = "No Match",
                  Class = "No Match",
                  Order = "No Match",
                  Family = "No Match",
                  Genus = "No Match",
...
write.csv(df.new, file.path(path_results,
                            "COI_nested_taxonomy_boldigger.csv"))
...
write.csv(track,
          file.path(path_results, "track_reads.csv"),
          row.names = FALSE)

tax <- as.data.frame(tax)
tax <- cbind(colnames(seqtab_nochim), tax)
...
write(asv_fasta_no_contam,
      file.path(path_results, "ASVs-no-contam.fa"))
write.csv(asv_tab_no_contam,
          file.path(path_results, "ASVs_counts-no-contam.csv"),
          row.names = FALSE)
write.csv(asv_tax_no_contam,
          file.path(path_results, "ASVs_taxonomy-no-contam.csv"),
          row.names = FALSE)
